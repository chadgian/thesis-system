...
window.onload = function () {
    ...
    document.getElementById('secondDocuments').addEventListener('change', function(event){
        ...
        let currentFolder = '';
        let currentFolderList = null;
        let fileItemList = null;

        for (let i = 0; i < filteredFiles.length; i++) {
            const parts = filteredFiles[i].webkitRelativePath.split('/');
            const fileFolder = parts[parts.length-2];
            const fileName = parts[parts.length-1];

            if (fileFolder !== currentFolder) {
                /*if (currentFolderList !== null) {
                    fileList2.appendChild(currentFolderList);
                }*/
                currentFolder = fileFolder;
                currentFolderList = document.createElement('p');
                currentFolderList.textContent = currentFolder;
                fileList2.appendChild(currentFolderList);
                fileItemList = document.createElement('ol');
                fileItemList.start = i+1;
            }
...
function secondOption(){
    ...
    document.getElementById('second-upload-button').style.transform = 'scale(0)';
    setTimeout(function() {
        document.getElementById('second-upload-button').style.display = 'none';
    ...
    function first(){
        if (firstCounter < filteredFiles.length){
            ...
            secondFormData.append("file", filteredFiles[firstCounter]);
            ...
            .then(data => {
                var firstResult = '';
                var secondCounter = firstCounter+1;
                ...
                function second(){
                    if (secondCounter < filteredFiles.length){
                        ...
                        secondFormData2.append('files', filteredFiles[secondCounter]);
                        ...
                        .then(response => response.json())
                        .then(newData => {
                            tableCell1 = document.getElementById(firstCounter+''+secondCounter);
                            tableCell2 = document.getElementById(secondCounter+''+firstCounter);
                            ...
                            tableCell1.textContent = result;
                            tableCell2.textContent = result;
